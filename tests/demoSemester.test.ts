...
test("demo semester seed is deterministic and shaped for store capture", () => {
  ...
  assert.equal(first.assignments.filter((assignment) => assignment.type !== "exam").length, 20);
  assert.equal(first.assignments.filter((assignment) => assignment.type === "exam").length, 3);
...
test("demo semester includes overdue, this-week, and mixed-review states", () => {
  ...
  const weekStart = new Date("2026-10-05T00:00:00-04:00").getTime();
  const weekEnd = new Date("2026-10-11T23:59:59-04:00").getTime();
  ...
  assert.equal(dueThisWeek.length, 5);
