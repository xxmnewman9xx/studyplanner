...
export async function recordReviewEvent(trigger: ReviewTrigger): Promise<ReviewEventResult> {
  ...
  if (shouldAsk) {
    next.promptedAt = new Date().toISOString();
  }
  await saveJson(reviewStateKey, next);
  if (shouldAsk) await requestNativeStoreReview(next);
  return { shouldPrompt: shouldAsk, promptedAt: next.promptedAt };
}

async function requestNativeStoreReview(state: ReviewPromptState) {
  try {
    ...
    state.requestedStoreReviewAt = new Date().toISOString();
    await saveJson(reviewStateKey, state);
  } catch {
    // A review request must never block the study flow.
  }
}
