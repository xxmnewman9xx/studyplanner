...
| 51 | `expo-iap` Lifetime product type audit | Passed | Installed `expo-iap` normalizes `in-app` as canonical and keeps `inapp` as deprecated alias | Missing Lifetime returned product is not caused by the app's `type: "in-app"` fetch |
| 51 | StoreKit price/source comparison | Blocked with evidence | Local `.storekit` prices are `$4.99` monthly and `$29.99` yearly; products-loaded screenshot shows `$3.99` monthly and `$24.99` yearly | Treat current products-loaded screenshot as returned store-product proof, not local StoreKit Testing proof |
| 51 | StoreKitTest local proof harnesses | Failed and documented | Standalone simulator Swift, temporary iOS XCTest, and temporary macOS SwiftPM XCTest all failed on local StoreKit service/framework limits; temporary native target was backed out | Use Xcode Run with StoreKit config or App Store Connect sandbox for real transaction proof |
| 51 | Regression checks after StoreKit proof-path update | Passed / failed as intended | `npm run test` passed 55/55; `npm run verify:goal92` remained GOAL-OPEN with 3 blockers; `npm run verify:submission` remained NO-SUBMIT with 6 blockers and 1 warning | Documentation is current and the gates still protect the unresolved external proof |
| 52 | Products-loaded observation verifier | Passed | `verify:goal92` and `verify:submission` now require the products-loaded PNG to be backed by written returned-product and Lifetime-caveat observations in `storekit-sandbox-attempt.md`; `npm run typecheck` passed, `npm run test` passed 55/55, and both gates still fail only on real blockers | Prevents screenshot existence alone from overstating StoreKit readiness |
| 53 | iPhone Simulator StoreKitTest framework path | Passed with caveat | Confirmed `StoreKitTest.framework` exists under the iPhone Simulator SDK at `$(SDKROOT)/Developer/Library/Frameworks`; a temporary Swift simulator binary typechecked and compiled against it | Standalone simulator execution still failed to connect to `com.apple.storekitd`, so this is setup evidence only |
| 53 | App-hosted iOS XCTest StoreKit proof attempt | Failed and documented | A temporary ignored/generated `StudyPlannerSyllabusAITests` target built and executed with app-hosted test wiring, bundled `.storekit` variants, and `SKTestSession(configurationFileNamed:)`; both StoreKit tests failed before purchase assertions with `SKInternalErrorDomain Code=3` | Local XCTest transaction proof remains blocked; use Xcode Run StoreKit Testing or App Store Connect sandbox |
| 53 | StoreKit proof-path cleanup | Passed | Reverted tracked `.storekit` version/scheme experiments and kept the attempt documented in `storekit-sandbox-attempt.md`, `STOREKIT_TESTING_RUNBOOK.md`, and the evidence ledger | No StoreKit proof was claimed; blocker remains honest |
