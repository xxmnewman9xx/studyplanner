...
- Submission verifier proof: `npm run verify:submission` passes local screenshot export checks, English ASO copy audit, localized ASO structural audit, iOS archive source preflight audit, and the source-level VoiceOver audit, then fails honestly until external StoreKit/support/App Store Connect/VoiceOver/archive proof is supplied.
