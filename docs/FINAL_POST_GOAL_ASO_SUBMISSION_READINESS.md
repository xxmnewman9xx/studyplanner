...
- Submission readiness is now guarded by `npm run verify:submission`, which passes local screenshot export checks, English ASO copy checks, products-loaded paywall screenshot check, the clean source-level VoiceOver audit, and local simulator VoiceOver traversal, then returns **NO-SUBMIT** until IAP env IDs, support URL, StoreKit sandbox proof, App Store Connect upload acceptance, and signed archive entitlements are supplied.
