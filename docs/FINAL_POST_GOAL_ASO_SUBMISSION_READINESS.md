...
- Submission readiness is now guarded by `npm run verify:submission`, which passes local screenshot export checks, English ASO copy checks, products-loaded paywall screenshot and written observation checks, the clean source-level VoiceOver audit, and local simulator VoiceOver traversal, then returns **NO-SUBMIT** until IAP env IDs, support URL, StoreKit sandbox proof, App Store Connect upload acceptance, and signed archive entitlements are supplied.
...
- Paywall failure proof was captured honestly as `38-paywall-product-load-failure.png`; products-loaded proof is now captured as `37-paywall-products-loaded.png` with returned Yearly Plus and Plus Monthly products. The visible product prices differ from the local `.storekit` file, so this is not local StoreKit Testing proof. Lifetime transaction availability and purchase/restore success remain unproven.
...
StoreKit purchase/restore proof, support URL, manual App Store Connect screenshot upload acceptance, full UI localization/native review, signed archive entitlement check, and final simulator QA. The `npm run verify:submission` command tracks these as explicit blockers instead of relying on prose; it now also runs the English ASO copy audit, localized ASO structural audit, iOS archive source preflight audit, StoreKit source handoff audit, products-loaded screenshot check, products-loaded returned-product/Lifetime-caveat observation check, source-level VoiceOver audit, and local simulator VoiceOver traversal, but those do not replace App Store Connect acceptance, native review, signed archive inspection, StoreKit sandbox proof, or Lifetime transaction proof. Failed standalone simulator, iOS XCTest, and macOS XCTest StoreKitTest attempts are recorded in `storekit-sandbox-attempt.md`; the next valid path is Xcode Run with the StoreKit configuration attached or App Store Connect sandbox testing.

The capture-mode rebuild solved the original deep-link capture problem, installed small/medium WidgetKit screenshots now exist, app icon proof exists, native widget empty/needs-check states exist, reminders proof exists, settings proof exists, restore access proof exists, iPad raw proof exists, partial localized/date proof exists, contrast-token proof exists, local App Store-sized export proof exists, completion/edit/add refresh is proven, day-boundary behavior is code/build proven, core action large-text proof now exists, source-level VoiceOver readiness is clean, local simulator VoiceOver traversal is recorded, and Manual Add/Edit Item/filtered Calendar/import-path capture states are now real. The screenshot set is closer to upload-ready, but final App Store readiness still needs manual App Store Connect upload acceptance and StoreKit/localization proof. Missing proof includes restore purchase success/sandbox proof, translated UI/string extraction/native review, manual screenshot upload acceptance, and StoreKit transaction completion proof.
