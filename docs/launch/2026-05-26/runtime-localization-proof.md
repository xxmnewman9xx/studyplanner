...
- `scripts/check-localization-completeness.mjs` scans the launch files plus `ModeToggle`, `TodayScreen`, `PlanScreen`, `CoursesScreen`, and `FocusScreen`, verifies required keys in every locale, rejects selected hard-coded launch strings, and rejects non-English values that silently equal `en-US` except intentional product/platform terms.
...
Actual result after the Focus localization patch:
...
Release localization is not complete. A direct hard-coded JSX text audit no longer flags `TodayScreen`, `PlanScreen`, `CoursesScreen`, or `FocusScreen`, but still finds 84 candidate app-owned English matches in launch-relevant surfaces including:
...
The current gate proves all existing static runtime keys are populated and the smoked Today/Calendar/Classes/Focus/Scan/Plus/native shell is localized. It does not prove every runtime string in the app has been replaced with reviewed translations.

## RTL Risk

Arabic is marked `direction: "rtl"` and the app shell receives RTL direction. Current Arabic screenshots show right-to-left Today/Calendar/Classes/Focus/Scan/Plus shell ordering and localized theme labels, but release still has RTL risk because:
