...
- `src/screens/AssignmentDetailScreen.tsx` no longer renders app-owned assignment detail/edit/trust text directly for validation alerts, header labels, task-state copy, kind/priority/status/source labels, reminder/confidence meta, review callouts, checklist empty state, and save/archive actions; it uses `assignment_detail.*` with existing tab/runtime keys and locale-aware date formatting.
...
- `src/components/PremiumGate.tsx` no longer renders app-owned subscription lock copy directly; it uses `premium_gate.*` runtime keys.
- `App.tsx` no longer renders app-owned quick-add, course-add, import-apply, reminder, calendar, sidebar subscription, or device-permission alert copy directly; those paths use `app.*`, `common.*`, `import.*`, `paywall.*`, and tab runtime keys.
...
- `scripts/check-localization-completeness.mjs` scans the launch files and audited runtime surfaces: `ModeToggle`, `TodayScreen`, `PlanScreen`, `CoursesScreen`, `FocusScreen`, `GradesScreen`, `MoreScreen`, `ImportScreen`, `AssignmentDetailScreen`, `NotesScreen`, `PremiumGate`, `widgetSnapshot`, and `AppleComponents`. It verifies required keys in every locale, rejects selected hard-coded launch strings, and rejects non-English values that silently equal `en-US` except intentional product/platform terms.
...
- `24-paywall.png` for `ar`, `de`, `ja`, and `zh-Hans` shows localized subscription/paywall shell, localized theme labels, two loaded products, yearly `$24.99`, monthly `$3.99`, restore visible, and legal links visible.
...
Do not upload a new build from this state. App-owned launch-critical runtime localization is now wired and gated, with current native Release proof for Arabic onboarding and existing native Release locale proof for Today/Calendar/Classes/Focus/Grades/More/Widgets/Scan/Review/Paywall. Release remains blocked until Arabic RTL risk is accepted or fixed and StoreKit product metadata localizations are verified in App Store Connect/TestFlight.
