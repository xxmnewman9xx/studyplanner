...
- The reviewed runtime catalog now contains 266 flattened entries per locale.
- Navigation, onboarding headline/copy, Today launch surface, Scan headline/copy/source picker/review shell, photo-disabled errors, Plus paywall headline/copy/features/status/legal/CTA shell, widget sync fallback, and the app theme toggle are wired through runtime keys.
...
- `scripts/check-localization-completeness.mjs` scans the launch files plus `ModeToggle` and `TodayScreen`, verifies required keys in every locale, rejects selected hard-coded launch strings, and rejects non-English values that silently equal `en-US` except intentional product/platform terms.
...
Actual result after the Today localization patch:
...
Release localization is not complete. A direct hard-coded JSX text audit no longer flags `TodayScreen`, but still finds 117 candidate app-owned English matches in launch-relevant surfaces including:
...
The current gate proves all existing static runtime keys are populated and the smoked Today/Scan/Plus/native shell is localized. It does not prove every runtime string in the app has been replaced with reviewed translations.

## RTL Risk

Arabic is marked `direction: "rtl"` and the app shell receives RTL direction. Current Arabic screenshots show right-to-left Today/Scan/Plus shell ordering and localized theme labels, but release still has RTL risk because:
