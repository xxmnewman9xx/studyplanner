...
Because strict edit, file-picker upload, restarted creative generation, first-three outcome generation, dimension-lock prompting, compensation prompting, the fresh final-prefix canary, and the Mac app viewer share-sheet export routes all fail the required dimensions, mass-sending the full 119-item queue would produce likely rejected outputs. Continue only after one GPT Image 2.0 candidate is retrievable as a raw exact `1242x2688` PNG and passes visual QA.
