...
| --- | --- | --- | --- | --- |
| G92-B01 | Final readiness report marks 9.2 reached | doc-state-after-proof | docs/FINAL_9_2_READINESS_REPORT.md | Leave the report at No until products-loaded paywall, StoreKit sandbox/restore, and VoiceOver traversal proof exist; then update and re-run the gate. |
...
| G92-B03 | Products-loaded paywall proof exists | external-storekit-screenshot | artifacts/post-goal-aso-submission/37-paywall-products-loaded.png | Follow docs/STOREKIT_TESTING_RUNBOOK.md, launch a StoreKit-enabled build, capture the paywall products-loaded state, and re-run the gate. |
| G92-B04 | StoreKit sandbox purchase/restore proof exists | external-storekit-sandbox | artifacts/post-goal-aso-submission/external-proof/storekit-sandbox-proof.md | Run the StoreKit Testing or sandbox flow from docs/STOREKIT_TESTING_RUNBOOK.md, record product IDs and restore output, then re-run npm run verify:goal92. |
| G92-B05 | Full VoiceOver traversal proof exists | manual-accessibility-traversal | artifacts/post-goal-aso-submission/external-proof/voiceover-traversal.md | Follow docs/VOICEOVER_TRAVERSAL_RUNBOOK.md with VoiceOver enabled on simulator/device, record real traversal results, and re-run npm run verify:goal92. |
