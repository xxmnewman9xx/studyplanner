...
- `npm run test` now includes Monday-start calendar coverage for `en-GB`, week-start helper coverage for `en-US`, `en-GB`, `de-DE`, and `ar-EG`, plus 24-hour due-date formatting coverage for `fr-FR` and `en-GB`; latest run: 35/35 tests.
