...
| --- | --- | --- | --- | --- | --- |
| Accessibility | 8.4 | 8.5 | Many small touch targets and reduced motion gaps were fixed; Today and four core action screens no longer catastrophically break at an accessibility text size; high-traffic planner visuals now expose VoiceOver labels; full VoiceOver traversal and color-only graph alternatives still need a full pass. | 44pt/hitSlop, labels, darker semantic colors, reduced-motion guard, targeted max font scale for compact shared/core action components. | Quick wins, large-text proof, and visual-surface labels implemented |
