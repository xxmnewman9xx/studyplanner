...
| --- | --- | --- | --- | --- | --- |
| Accessibility | 8.8 | 8.5 | Many small touch targets and reduced motion gaps were fixed; Today and four core action screens no longer catastrophically break at an accessibility text size; high-traffic planner visuals now expose VoiceOver labels; foreground-bearing theme/class/widget colors now have automated contrast coverage; `npm run audit:voiceover` reports 104/104 scanned interactive elements with explicit labels and roles; full VoiceOver traversal and color-only graph alternatives still need a full pass. | 44pt/hitSlop, labels, darker semantic colors, reduced-motion guard, targeted max font scale for compact shared/core action components, contrast regression guard, source-level VoiceOver audit. | Quick wins, contrast guard, large-text proof, visual-surface labels, and source-level VoiceOver audit implemented |
...
- `npm run audit:voiceover` now writes `docs/VOICEOVER_READINESS_AUDIT.md`; the latest audit scanned 21 UI files and 104 interactive elements, with 0 missing explicit labels, 0 missing roles, and 18 recommended hints still worth manual review.
...
Remaining accessibility proof needed for final submission confidence: real VoiceOver traversal, recommended hint review, localized/large-text spot checks, and simulator proof that color-independent graph/status alternatives read well in context.
