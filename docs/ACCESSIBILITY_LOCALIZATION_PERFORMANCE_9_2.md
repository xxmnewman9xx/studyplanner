...
- `npm run typecheck` and `npm run test` passed after the accessibility changes; latest run: 44/44 tests, including `tests/accessibilitySource.test.ts`, `tests/themeContrast.test.ts`, current-date capture seed coverage, widget day-boundary coverage, and capture-state source coverage.
...
- `npm run test` now includes Monday-start calendar coverage for `en-GB`, week-start helper coverage for `en-US`, `en-GB`, `de-DE`, and `ar-EG`, plus 24-hour due-date formatting coverage for `fr-FR` and `en-GB`; latest run: 44/44 tests.
