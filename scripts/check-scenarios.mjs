...
assert(app.includes('{ id: "import", label: "Scan"') && app.includes('{ id: "plan", label: "Plan"') && app.includes('{ id: "more", label: "Widgets"') && app.includes('return tab === "more" ? "Widgets" : fallback'), "Free tab bar must expose Scan, Plan, Classes, and Widgets.");
...
assert(!upgrade.includes("Continue free to Scan"), "Hard paywall must not expose a continue-free escape.");
assert(today.includes('label="Scan syllabus"') && today.includes("onPress={onOpenScan}"), "Free Scan starter CTA should route to scan/import.");
...
assert(onboarding.includes('id: "scan"') && onboarding.includes('id: "review"') && onboarding.includes('id: "today"') && onboarding.includes('id: "widgets"'), "Onboarding must preview the current Scan, Review, Today, and Widgets core loop.");
...
assert(upgrade.includes("StudyPlanner Plus") && upgrade.includes("Unlock syllabus scans, Home Screen widgets, reminders, and calendar sync."), "Hard paywall must clearly require Plus before main app access.");
assert(upgrade.includes("More syllabus scans") && upgrade.includes("More widget styles") && upgrade.includes("Reminders + calendar"), "Paywall should sell Plus leverage through real product surfaces.");
...
assert(more.includes("Add your plan to the Home Screen.") && more.includes("Choose one widget type, pick a look, and save it."), "Widget surface should lead with a useful native widget workbench.");
assert(more.includes("Today") && more.includes("Upcoming") && more.includes("This week's load"), "Widget templates should be student-outcome first.");
...
assert(more.includes("Save Today preset") && more.includes("Unlock this preset"), "Widget surface may save basic presets while gating advanced widgets.");
