...
const xcodeProject = readIfExists("ios/StudyPlannerSyllabusAI.xcodeproj/project.pbxproj");
...
expect(!xcodeProject || (!xcodeProject.includes("CURRENT_PROJECT_VERSION = 52;") && (xcodeProject.match(/CURRENT_PROJECT_VERSION = 56;/g) || []).length >= 4), "native app and widget project versions must remain 56 when ios project files are present");
