...
const OUTPUT_PATH = "qa/back-to-school-2026/widget-capture-ingest.json";
const OUTPUT_ROOT = "qa-screenshots/back-to-school-2026-native";

const args = process.argv.slice(2);
const apply = args.includes("--apply");
const strict = args.includes("--strict");
const sourceRoot = valueArg("--source") || process.env.STUDYPLANNER_WIDGET_CAPTURE_SOURCE || OUTPUT_ROOT;
...
function parseRunbookTable(markdown: string, heading: string): WidgetCaptureRow[] {
  ...
  return section
    ...
    .filter((cells) => cells.length >= 4 && cells[0].startsWith("widget-"))
    .map((cells) => ({
      id: cells[0],
      state: cells[1],
      appearance: cells[2],
...
function findCandidate(row: WidgetCaptureRow, files: string[]) {
  const allowedExtensions = new Set([".png", ".jpg", ".jpeg"]);
  return files.find((file) => {
    const extension = extname(file).toLowerCase();
    if (!allowedExtensions.has(extension)) return false;
    const normalized = file.toLowerCase();
    return normalized.includes(row.id.toLowerCase());
  }) || null;
}

function outputPathFor(row: WidgetCaptureRow, sourcePath: string) {
...
function validateCandidate(row: WidgetCaptureRow, sourcePath: string | null): Candidate {
  if (!sourcePath) {
    return {
      captureId: row.id,
      sourcePath: null,
      outputPath: null,
      status: "missing",
      reason: "No source image found. Name the screenshot with the capture ID, for example widget-03-exam-heavy-medium.png.",
    };
  }
  ...
  if (stat.size <= 5000) {
    return {
      ...
      outputPath,
      status: "invalid",
      reason: "Image is too small to be a usable WidgetKit screenshot.",
      bytes: stat.size,
...
function mergeManifest(rows: WidgetCaptureRow[], candidates: Candidate[]) {
  ...
  const nonWidgetEntries = (existing.entries || []).filter((entry) => !entry.captureId?.startsWith("widget-"));
  const rowById = new Map(rows.map((row) => [row.id, row]));
  const widgetEntries: NativeCaptureEntry[] = candidates
    ...
    .map((candidate) => {
      ...
      return {
        ...
        status: "captured",
        device: "native-widgetkit-placement",
        buildNumber: "release-or-testflight",
        appearance: row?.appearance,
        locale: "en-US",
        accessibilitySettings: row?.appearance || "default",
        sourceStateFixture: row?.state,
        reviewerNotes: "Registered from real WidgetKit Home Screen or Lock Screen placement capture.",
  ...
  const captured = widgetEntries.length;
  ...
  const manifest: NativeCaptureManifest = {
    ...
    release: "Back to School with AI",
    outputRoot: OUTPUT_ROOT,
    entries: [...nonWidgetEntries, ...widgetEntries],
    nativeWidgetPlacement: {
      status,
      captured,
      total: rows.length,
      auditPath: OUTPUT_PATH,
      note: "Widget entries must be real WidgetKit placement captures, not in-app previews or composites.",
    },
  };
...
function main() {
  ...
  const rows = parseRunbookTable(runbook, "## Native Widget Capture Matrix");
  ...
  const payload = {
    ...
    candidates,
    rules: [
      "Only real WidgetKit Home Screen or Lock Screen placement screenshots should be registered.",
      "Do not register in-app widget previews, Expo Go screenshots, web smoke screenshots, or composited Home Screen artwork.",
      "Name source files with the capture ID from the native runbook, such as widget-06-tinted-medium.png.",
      "Use --apply only after reviewing the source images.",
    ],
    commands: {
      plan: "npm run plan:back-to-school-widget-captures",
      applyFromDefaultFolder: "npm run register:back-to-school-widget-captures",
      applyFromCustomFolder: "npm run register:back-to-school-widget-captures -- --source /path/to/widget/screenshots",
  ...
  if (strict && (rows.length === 0 || found.length !== rows.length || invalid.length > 0)) {
    console.error("Back-to-School WidgetKit capture registration incomplete:");
  ...
  console.log(
    `Back-to-School WidgetKit capture registration ${status}. ` +
