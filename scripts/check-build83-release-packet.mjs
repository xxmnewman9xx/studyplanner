...
const copy = json("docs/launch/back-to-school-2026/build83-five-slide-copy.json");
const metadata = json("docs/launch/back-to-school-2026/build83-event-nomination-metadata.json");
const app = json("app.json").expo;
const capture = readFileSync("scripts/capture-build83-store-media.mjs", "utf8");
const queue = readFileSync("scripts/create-build83-chatgpt-preview-queue.mjs", "utf8");
...
assert.equal(app.ios.buildNumber, "83");
...
assert.equal(metadata.release.buildNumber, "83");
...
for (const fragment of ["2.0.8 (83)", "nonce-bound-app-acknowledgement-v5", "installedBundleVerified: true", "uploadAuthorized: false"]) {
  assert.ok(capture.includes(fragment), `capture invariant missing: ${fragment}`);
}
for (const fragment of ["ChatGPT Mac app", "GPT Image 2.0", "items.length !== 170", "immutable real StudyPlanner Build 83", "Do not redraw", "uploadAuthorized: false"]) {
...
assert.equal(existsSync("scripts/compose-build83-store-media.mjs"), false, "Build 83 must use ChatGPT Mac prompts, not a local compositor");

console.log(`Build 83 packet passed: 17 locales, 85 localized messages, 170 ChatGPT Mac jobs, metadata score ${score.toFixed(2)}/10.`);
