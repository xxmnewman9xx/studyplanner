...
const prototypeUISource = readFileSync("src/components/PrototypeUI.tsx", "utf8");
...
for (const phrase of forbiddenSampleWidgetData) {
  expect(!prototypeUISource.includes(phrase), `shared widget preview must not default to fake coursework: ${phrase}`);
  expect(!moreScreenSource.includes(phrase), `legacy widget screen must not pass fake coursework into previews: ${phrase}`);
}

const previewStart = appSource.indexOf("function NativeHomeWidgetPreview");
const previewEnd = appSource.indexOf("function WidgetsScreen");
const previewSource = previewStart >= 0 && previewEnd > previewStart ? appSource.slice(previewStart, previewEnd) : "";
