...
assert.match(appSource, /const status = await syncNativeWidgets\(widgetSyncData, widgetCopyFor(?:, \{[\s\S]{0,400}?\})?\);\s*if \(status\.state !== "synced"\)/, "automatic widget sync must inspect resolved failure states");
